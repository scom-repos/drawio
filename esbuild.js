...
async function buildGlobalModule() {
  ...
  await Promise.all(
    GlobalBuildModuleList.map((v) => writeContent(v.distFilePath, v.moduleName))
  );
...
async function buildLocalModule() {
  require("esbuild")
    .build({
      entryPoints: ["src/staking/index.tsx"],
      external: [
        "@drawio/assets",
        "@ijstech/components",
        "@ijstech/eth-wallet",
